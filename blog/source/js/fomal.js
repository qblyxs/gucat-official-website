...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Gucat🥝")[0];
}
...
// 引用jq
document.write('<script src="https://unpkg.zhimg.com/jquery@latest/dist/jquery.min.js"></script>');
/* 欢迎信息 start */
//get请求
$.ajax({
  type: 'get',
  ...
  data: {
    key: '2IZBZ-AZBWB-L5TUK-NNGZV-ITFS2-DBBEO',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(103.88194, 30.81444, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里记录你自己的经纬度
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Gucat🥝 %c 你正在访问Gucat🥝の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Gucat🥝") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Gucat🥝的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://mirrors.gucat.vip/api/public/dl/T1IFvlYf/svg/小屋-科研摸鱼中.svg?inline=true' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://mirrors.gucat.vip/api/public/dl/T1IFvlYf/svg/小屋-下班休息啦.svg?inline=true' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
// 随机美图cdn.seovx.com
let seovx = "url(http://cdn.seovx.com/?mom=302)";
// picsum随机
